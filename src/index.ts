 ...
 * telnet_connect 핸들러
 */
async function handleConnect(args: { host: string; port: number; timeout?: number }) {
 ...
 * telnet_send 핸들러
 */
async function handleSend(args: { sessionId: string; command: string }) {
  // 세션 조회
 ...
 * telnet_read 핸들러
 */
async function handleRead(args: { sessionId: string; waitMs?: number; encoding?: string }) {
  // 세션 조회
  const session = sessionManager.getSession(args.sessionId);
  if (!session) {
    throw new Error('Session not found');
 ...
 */
async function handleDisconnect(args: { sessionId: string }) {
  try {
    // 세션 삭제 (연결 종료 포함)
