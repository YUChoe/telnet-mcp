...
export class TelnetSession {
     ...
     */
    async connect(host, port, timeout = DEFAULT_CONFIG.TIMEOUT) {
    ...
    setupDataHandler() {
        if (!this.session.socket)
            return;
        this.session.socket.on('data', (data) => {
            // IAC 시퀀스 처리
     ...
     */
    async readResponse(waitMs, encoding = DEFAULT_CONFIG.DEFAULT_ENCODING) {
        // 대기 옵션이 있으면 대기
        if (waitMs && waitMs > 0) {
